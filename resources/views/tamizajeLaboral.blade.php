...
<title>Trazabilidad de Tamizajes · GESI</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;600;700;900&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;600&display=swap" rel="stylesheet">
...
<style>
  :root{
    ...
    --shell-surface-2:#EEF2F9;
    --shell-border:#E2E8F0;
    --shell-border-strong:#CBD5E1;
    --shell-red:#2563EB;
    --shell-red-dim:#DBEAFE;
    --shell-red-glow:rgba(37,99,235,.35);
    --shell-text:#14213D;
    --shell-muted:#64748B;
    --shell-muted-2:#94A3B8;
    --shell-head:'Space Grotesk', sans-serif;
    --shell-sans:'Inter', sans-serif;
  ...
  [data-theme="dark"]{
    ...
    --shell-border:#334155;
    --shell-border-strong:#475569;
    --shell-red:#60A5FA;
    --shell-red-dim:#1E3A5F;
    --shell-red-glow:rgba(96,165,250,.35);
    --shell-text:#F1F5F9;
    --shell-muted:#94A3B8;
    --shell-muted-2:#64748B;
  ...
  *{box-sizing:border-box;}
  ...
  .wrap{max-width:1320px;margin:0 auto;padding:2.25rem clamp(1.5rem,4vw,4rem) 4rem;}

  /* ---------- Topbar / theme toggle / tokens (igual a las demás vistas ODIN) ---------- */
  .topbar{
  ...
  /* ---------- Page header / botones / kpis / cards (portados de shell.css, recoloreados en azul) ---------- */
  .adm-page-header{
    display:flex; align-items:flex-end; justify-content:space-between; flex-wrap:wrap;
    gap:1rem; margin-bottom:1.25rem; padding-bottom:1.5rem; border-bottom:1px solid var(--shell-border);
  }
  .adm-page-tag{
    font-family:var(--shell-mono); font-size:11px; letter-spacing:3px; text-transform:uppercase;
    color:var(--shell-red); display:block; margin-bottom:.5rem;
  }
  .adm-page-title{
    font-family:var(--shell-head); font-size:1.9rem; font-weight:900; text-transform:uppercase;
    letter-spacing:.5px; margin:0; color:var(--shell-text);
  }
  .adm-btn{
    font-family:var(--shell-sans); font-size:.82rem; font-weight:600; border-radius:6px;
    padding:.55rem 1.4rem; border:1px solid transparent; cursor:pointer; transition:.2s;
  }
  .adm-btn-primary{background:var(--shell-red); color:#fff;}
  .adm-btn-primary:hover{box-shadow:0 0 16px var(--shell-red-glow);}
  .adm-btn-primary:disabled{opacity:.5; cursor:not-allowed; box-shadow:none;}
  .adm-btn-ghost{
    background:transparent; border-color:var(--shell-border-strong); color:var(--shell-muted);
    text-decoration:none; display:inline-flex; align-items:center;
  }
  .adm-btn-ghost:hover{color:var(--shell-text); border-color:var(--shell-text);}
  .adm-kpi{
    background:var(--shell-surface); border:1px solid var(--shell-border); border-radius:10px;
    padding:1.5rem 1.75rem; transition:all .25s ease; box-shadow:var(--shadow);
  }
  .adm-kpi:hover{transform:translateY(-4px); border-color:var(--shell-border-strong);}
  .adm-kpi.is-accent{
    border-color:rgba(37,99,235,.3); background:linear-gradient(180deg, var(--shell-red-dim), transparent);
  }
  .adm-kpi-label{
    font-family:var(--shell-mono); font-size:10px; letter-spacing:1.5px; text-transform:uppercase;
    color:var(--shell-muted); margin-bottom:.75rem;
  }
  .adm-kpi-value{font-family:var(--shell-head); font-size:2.4rem; font-weight:900; line-height:1; color:var(--shell-text);}
  .adm-card{
    background:var(--shell-surface); border:1px solid var(--shell-border); border-radius:10px;
  ...
  /* =========================================================================
     ...
     otras vistas. Usa las variables --shell-* definidas arriba.
     ========================================================================= */
  .tl-page-desc{
    font-size:.83rem;color:var(--shell-muted);margin:0 0 1.5rem;max-width:760px;line-height:1.5;
  }
  ...
  .tl-footer-note{margin-top:1.4rem;font-size:.68rem;color:var(--shell-muted-2);text-align:center;font-family:var(--shell-mono);line-height:1.7;}
...
<div class="wrap">
  ...
  <div id="tl-results-area" style="display:none;">
      ...
      <div class="tl-footer-note">
        Cruce basado en número de documento normalizado (solo letras y dígitos). Los documentos sin ese campo diligenciado no se pueden validar.<br>
...
const tlPanelUpload = document.getElementById('tl-panel-upload');
...
function showBlock(message){
  ...
  tlPanelUpload.style.pointerEvents = 'none';
  tlPanelUpload.style.opacity = '0.5';
}
...
async function chargeTokens(rowsProcessed){
  ...
  if(currentTokens <= 0){
    showBlock('Te quedaste sin tokens tras procesar este archivo. Adquiere más tokens para seguir usando el validador.');
  }
}
...
(function () {
  ...
  btnExport.addEventListener('click', async () => {
    ...
    try {
      ...
      ws2.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1D4ED8' } };
      const byMod = {};
      GesiCore.SOURCE_MODULES.forEach(m => { byMod[m.key] = { label: m.label, total: 0, ok: 0 }; });
      state.filtered.forEach(r => {
        if (!byMod[r.moduloKey]) return;
