...
<head>
    ...
    <!-- Custom stylessheets -->
    @vite(['resources/css/env/shell.css', 'resources/css/env/components.css', 'resources/css/env/main-env.css', 'resources/css/global.css', 'resources/css/env/review-files.css'])

    @if(Request::is('review/prepare-delivery'))
        @vite('resources/css/env/prepare-delivery.css')
    @endif

    @if(Request::is('env/traceability'))
        @vite('resources/css/env/traceability.css')
    @endif
</head>

<body id="page-top" class="env-body">
    <div class="env-shell">
        ...
        {{-- ── NAVBAR ── --}}
        <nav class="env-nav" id="envNav">
            <a class="env-nav-brand" href="{{ route('app.home') }}">
                <img src="{{ asset('img/logo_temp.png') }}" alt="Trakio">
                Trak<span>io</span>
            </a>

            <button class="env-nav-toggle" id="envNavToggle" type="button" aria-label="Abrir menú">
                <i class="fa-solid fa-bars"></i>
            </button>

            <div class="env-nav-links" id="envNavLinks">
                <a class="env-nav-link {{ Request::is('home') ? 'is-active' : '' }}" href="{{ route('app.home') }}">
                    <i class="fas fa-tachometer-alt"></i> Dashboard
                </a>

                <a class="env-nav-link {{ Request::is('env/users') ? 'is-active' : '' }}" href="{{ route('env.users') }}">
                    <i class="fa-solid fa-user-tie"></i> Usuarios
                </a>

                {{-- ── CONSECUTIVOS ── --}}
                <div class="env-nav-dropdown">
                    <a class="env-nav-link {{ Request::is('revisar-consecutivos') || Request::is('review/prepare-delivery') || Request::is('recibir-fichas/*') || Request::is('env/traceability') ? 'is-active' : '' }}" href="#" data-env-dropdown>
                        <i class="fa-solid fa-arrow-down"></i> Consecutivos <i class="fa-solid fa-chevron-down caret"></i>
                    </a>
                    <div class="env-nav-dropdown-menu">
                        <a class="env-nav-dropdown-item {{ Request::is('revisar-consecutivos') ? 'is-active' : '' }}" href="{{ route('reviewconsecutives.index') }}">
                            <i class="fa-solid fa-arrow-down me-1"></i> Solicitudes
                            @if ($pendingRequestsCount > 0)
                                <span class="badge rounded-pill bg-danger ms-1">{{ $pendingRequestsCount }}</span>
                            @endif
                        </a>
                        <a class="env-nav-dropdown-item {{ Request::is('review/prepare-delivery') ? 'is-active' : '' }}" href="{{ route('reviewconsecutives.prepare-delivery') }}">
                            <i class="fa-solid fa-arrow-right me-1"></i> Entrega GESI
                        </a>
                        <a class="env-nav-dropdown-item {{ Request::is('recibir-fichas/*') ? 'is-active' : '' }}" href="{{ route('env.reception.show') }}">
                            <i class="fa-solid fa-arrow-left me-1"></i> Recibir fichas
                        </a>
                        <a class="env-nav-dropdown-item" href="{{ route('env.devprof.show') }}">
                            <i class="fa-solid fa-arrow-left me-1"></i> Devolver Profesional
                        </a>
                        <a class="env-nav-dropdown-item {{ Request::is('env/traceability') ? 'is-active' : '' }}" href="{{ route('env.traceability') }}">
                            <i class="fa-solid fa-magnifying-glass me-1"></i> Trazabilidad
                        </a>
                    </div>
                </div>
                ...
                {{-- ── CERTIFICACIÓN (BETA) ── --}}
                {{-- <div class="env-nav-dropdown">
                    <a class="env-nav-link {{ Request::is('env/activities') || Request::is('documents/*') || Request::is('report-activities/generate') ? 'is-active' : '' }}" href="#" data-env-dropdown>
                        <i class="fa-regular fa-file-lines"></i> Certificación <small class="badge badge-beta">BETA</small> <i class="fa-solid fa-chevron-down caret"></i>
                    </a>
                    <div class="env-nav-dropdown-menu">
                        <a class="env-nav-dropdown-item {{ Request::is('env/activities') ? 'is-active' : '' }}" href="{{ route('activities.edit') }}">
                            <i class="fa-regular fa-file-lines me-1"></i> Perfiles
                        </a>
                        <a class="env-nav-dropdown-item {{ Request::is('documents/*') ? 'is-active' : '' }}" href="{{ route('documents.index') }}">
                            <i class="fa-solid fa-folder-open me-1"></i> Documentos
                        </a>
                        @if ($data_user)
                            <a class="env-nav-dropdown-item {{ Request::is('report-activities/generate') ? 'is-active' : '' }}" href="{{ route('report.activities.show') }}">
                                <i class="fa-solid fa-file-contract me-1"></i> Generar informe de actividades
                            </a>
                        @endif
                    </div>
                </div> --}}
            ...
            <div class="env-nav-actions">
                ...
                @endphp
                <div class="env-nav-rating d-none d-lg-flex">
                    <span class="stars">
                ...
                <div class="env-nav-dropdown">
                    <div class="env-nav-bell" data-env-dropdown id="alertsDropdown">
                        <i id="bell" class="fas fa-bell"></i>
                        <span class="dot" id="counter-notifications"></span>
                    </div>
                    <div class="env-nav-dropdown-menu scrollable-container" style="right:0; left:auto;" aria-labelledby="alertsDropdown"></div>
                </div>

                <div class="env-nav-dropdown">
                    <div class="env-nav-user" data-env-dropdown id="userDropdown">
                        <input type="hidden" value="" id="user_id">
                        <div class="env-nav-user-info">
                            <span class="env-nav-user-name">{{ $user->name }} {{ $user->last_name }}</span>
                            <span class="env-nav-user-role">Subred {{ $user->subnet->name }}</span>
                        </div>
                    ...
                    <div class="env-nav-dropdown-menu" style="right:0; left:auto;" aria-labelledby="userDropdown">
                        @if ($data_user)
                            <a class="env-nav-dropdown-item" href="{{ route('profile.edit') }}">
                        ...
                        <a class="env-nav-dropdown-item" href="#" id="btn-logout">
        ...
        <div class="env-main" id="main-container">
            @yield('main')
        </div>

        @if ($user->environment_id == 2 || $user->environment_id == 3)
            {{-- MODAL DATA USERS --}}
    ...
    <script>
        const routeReceiveReception = '{{ route('env.receive') }}';
    </script>
    ...
    <script>
        ...
        document.getElementById('envNavToggle').addEventListener('click', function () {
            document.getElementById('envNavLinks').classList.toggle('open');
        });

        // Dropdowns del navbar (consecutivos, certificación, alertas, usuario)
        document.querySelectorAll('[data-env-dropdown]').forEach(function (trigger) {
            trigger.addEventListener('click', function (e) {
                e.preventDefault();
                e.stopPropagation();
                var wrapper = trigger.closest('.env-nav-dropdown');
                var wasOpen = wrapper.classList.contains('open');
                document.querySelectorAll('.env-nav-dropdown.open').forEach(function (d) { d.classList.remove('open'); });
                if (!wasOpen) wrapper.classList.add('open');
            });
        });

        document.addEventListener('click', function (e) {
            document.querySelectorAll('.env-nav-dropdown.open').forEach(function (d) {
    ...
    <!-- Custom scripts -->
    @vite(['resources/js/documents/main-docs.js', 'resources/js/env/main-env.js', 'resources/js/env/dashboard.js', 'resources/js/assets/profile.js', 'resources/js/env/review-files.js']);
</body>
</html>
